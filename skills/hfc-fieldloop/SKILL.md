...
9. **Detect the completion signal.** Gating (a real screening/eligibility question from the instrument) is now the primary, preferred signal — it replaces the old status-column-name heuristic entirely, since reading the actual instrument is more reliable than pattern-matching a column name. Detection order:
   ...
   b. **roster** — the `roster_candidate` from step 1: a second file that looks like a target/planned-sample list. Only relevant when no credible gate was found.
...
6. **Draft the Summary narrative** — see `assets/summary_message_example.md` for the calibration target (a real, annotated example, calibrated against an actual field RA's daily Slack update). **Focus on the most recent day of data collection** — state that day's own numbers, not just a cumulative total — **but carry forward anything important that's still unresolved**, even if it didn't originate that day: before drafting, check whether the most recently dated `<MMDD>_HFCs.html` already in `hfc/outputs/` (if any) mentioned a finding that's still Open in `issue_tracking.xlsx` — if so, restate it briefly (e.g. "Same as yesterday: ...") rather than dropping it or re-deriving the whole explanation from scratch. Read `hfc/outputs/issues.csv` and draft a Slack-register message, **lead sentences first, a bullet list after**. Length follows however many real, distinct points the latest day and any carried-forward issues actually produce — don't pad, but don't compress real points just to keep it short:
   1. Names real places/entities/numbers with specificity — never "several schools," always "17 schools" / "School ID 4 Gashanga."
   2. **Lead — completion & reasons, outliers, duration, duplicates, enumerator roster, in that order, as plain sentences (not bullets):**
      a. The latest day's own attempted/completed/non-completed counts (M1's `stats$by_date`), alongside — not instead of — completion status vs. target overall (target-vs-actual, or primary/secondary composition — whichever signal applies; see A1) and completion % by group, defaulting to Treatment/Control when that grouping exists.
      b. **Reasons for non-completion**, when the confirmed signal is `gating` and `stats$by_reason` has any real entries — state the breakdown by name and count (e.g. "3 refused, 2 absent, 1 ineligible"), not just the raw non-completion total. Can fold into 2a's sentence when short, or its own sentence when longer.
      c. **Outliers worth a second look** (M6) — narrate a genuine flag in plain language with its actual value, and when you don't have enough local-context judgment to be sure it's wrong, pose it as a plausibility question rather than a flat assertion, same as a field RA would. Mark carry-forward status explicitly when it's the same flag from a prior day.
      d. M4 duration, mean **and** median, in minutes.
      e. M2 duplicates by name, with the real `issue_tracking.xlsx` path and an `@mention`-style placeholder for follow-up.
   ...
   3. **"Other issues:" bullet list, after the lead** — everything else worth surfacing: M5, M7-M13 issues, the data/media-presence gap statement at entity/group granularity (read `issues.csv`'s M11/M7 rows and translate into "were not available in N schools — schools X, Y, Z," not merely restating M11's raw one-line finding), and any other notable finding. One bullet per theme, same "name real entities/counts" rule as the lead. Never fabricate detail the pipeline's own findings/data don't support (e.g. photo content) — no check module inspects media content, only whether a media column is entirely empty (M11).
