...
### A1. Discover, gather instructions, then profile setup (Windows: Setup — 2 tabs, always; Entity & Country — up to 3 tabs, always)
...
7. **Now** read remaining context, in order, each informed by everything resolved in step 6:
   ...
   b. **The survey instrument/form, if found** — skip-logic (`relevant`), question structure; informs M10/M3/nested-logic judgment.
...
9. **Detect the completion signal** (`detect_completion_signal()` in `scripts/lib/profile_roles.R`) — up to three types, and more than one can be present at once:
...
10. **Window "Entity & Country" — up to 3 tabs, one atomic fact each, standard template:**
    ...
    - **Completion** (only when step 9 found a signal): states the guess (or the explicit conflict framing) plus the downstream effect — "every other check runs on that subset only." Data found and Instructions no longer share this window, so Entity/Country/Completion comfortably fit together without needing a separate follow-up window.
11. Persist everything confirmed (including any corrections from step 10) to `hfc/config/role_map.yaml`: `entity_id`, `entity_id_sep`, `entity_label`, `dup_key_extra`, `country_mode`, `country`/`country_col`/`country_timezone_map`, `timezone`, `qualitative_text_cols`, `completion_primary_signal`, `completion_status_col`, `completion_status_complete_values`, `completion_roster_candidate`, `completion_roster_key_col`, `completion_primary_secondary_col`, `completion_primary_value`, `context_doc_path` (from step 7a, so a rebuild doesn't need to re-discover it).
