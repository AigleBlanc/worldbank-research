...
18. **Report-wide sort:** every issues table sorts by date (most recent first), then enumerator, then unique ID — module sections stay in their fixed order (M1..M13), but there is no severity/worst-first ordering within a table. Dateless aggregate-level findings (group/enumerator-level, never tied to one day) sort last within their module. Every row-level finding matching the confirmed last date also appears in the dedicated Last Day tab (aggregate findings never can, since they have no date).
19. **Never ask a tab whose subject a prior answer already resolved.** Before presenting any A2 tab, check whether it's already settled — by A1's Instructions answer (free text or a hand-edited, non-blank value in `modules.yaml`/`role_map.yaml`). If resolved, skip the tab and state the fact once in chat instead of re-asking. Concrete example: declining GPS checks (`M8.on: no`) must also skip the GPS threshold tab (Dates, Variables & GPS window) *and* the Map focus tab (Media, Map & Grouping window) together — a map is meaningless with no GPS check behind it, and `write_html_report()` ties the map's rendering to `M8.on` for exactly this reason. If skipping drops a window to ≤1 remaining tab, backfill into a neighboring window rather than firing a near-empty one for a single leftover fact.
...
### A1. Discover, gather instructions, then profile setup (Windows: Setup — 2 tabs, always; Entity & Country — up to 2 tabs, always; Completion Signals — up to 3 tabs, conditional)
...
1. Run discovery in the configured Input Data Directory only — no searching elsewhere, but including any nested subfolders (`discover_project()`). This surfaces the data/form paths, a `roster_candidate` (a second file that looks like a target/planned-sample list, used by completion-signal detection in step 9), and a `context_doc` (a project description / Pre-Analysis Plan document — pdf/doc/docx/txt/md). No deep content reading yet — that starts in step 7, after Instructions.
...
4. **Write a blank `hfc/config/modules.yaml`** immediately:
   ...
   ```
   Writes a commented draft with every M1–M13 block present but unfilled (just its `desc:`/comment header) — no data dependency, safe to run before any role profiling. Its real path is stated in the Instructions tab below, inviting the user to pre-fill anything they already know (module on/off, thresholds, variable lists) directly in the file instead of waiting to be asked.
5. **Window "Setup" — 2 tabs, fires immediately, before any role profiling:**
   - **Data found**: sentence 1 states the real paths found (data file, form if any, and project description/PAP document if found); sentence 2 states row count, column count, and collection date range.
   - **Instructions**: *"Any specific instructions for this data or this HFC check session? This can be anything I should know before diving in, e.g. disabling checks, naming the country, or specific checks you want."* States the blank `modules.yaml`'s real path (from step 4) and invites pre-filling it directly, or leaving it for the agent to guide the rest of setup. Options: `"All good!"` / `Other`.
6. **Apply the Instructions answer before doing anything else:** interpret the free text as concrete facts/directives (e.g. "no GPS checks" → `M8.on: no`; a stated country → skip country inference and use it directly), and re-read `modules.yaml` for any hand-edited, non-blank values. Both become **already-resolved facts** — every tab from here on checks against them first instead of re-guessing and re-asking (Operating Principle 19).
...
8. **Country**: skip entirely if step 6 already stated it — state the fact once in chat instead. Otherwise, if an explicit country-name/code column exists (`shortlist_country_columns()`), use it directly and resolve its timezone(s) via `resolve_country_timezone_column()` (`scripts/lib/geo_timezone.R`). If not, read the sampled values from `shortlist_geography_signal_cols()` (district/region/village/site-name-like columns, plus a GPS bounding box if coordinates exist) and use your own general-knowledge judgment to state a best-guess country — **never infer this from the input folder's name/basename**, that's a report-title convenience only. Resolve the guessed country's timezone via `resolve_country_timezone()` — state it as abbreviation + city/country + UTC offset (e.g. "CAT (Blantyre, Malawi, UTC+2:00)"), never a bare IANA name — a resolved timezone is always shown back for confirmation, never trusted silently.
9. **Detect the completion signal.** Gating (a real screening/eligibility question from the instrument) is now the primary, preferred signal — it replaces the old status-column-name heuristic entirely, since reading the actual instrument is more reliable than pattern-matching a column name. Detection order:
   ...
   b. **roster** — the `roster_candidate` from step 1: a second file that looks like a target/planned-sample list. Only relevant when no credible gate was found. When roster wins, completion-by-strata (village, farmer group, Treatment/Control, etc.) reports the roster's own **target sample size** as the denominator — percent of the planned sample actually surveyed, not percent of submitted rows — so also check whether `modules$M1$group_vars` (the strata column(s) already detected in the survey data) exist by the same name in the roster file. If a strata column's name differs there (e.g. survey data has `village`, the roster has `village_name`), resolve the match by column-name similarity or by comparing raw values, and note it for the Completion Signals window below — silent when the names already match (the common case), no separate tab either way.
   ...
   If no gate, roster, or primary/secondary signal is found at all, there's nothing to confirm — skip the Completion Signals window entirely, and M1 falls back to its original row-missingness heuristic.
   d. **Replacement-sample structure — independent of a/b/c above, checked whenever a roster file exists at all** (even alongside a `gating` signal): inspect the roster's own columns for a Primary/Secondary-or-Replacement status column, a numeric position-in-the-replacement-queue column (only meaningful on non-primary rows), and a group column (e.g. farmer group — matched by raw value against `roles$group` in the survey data). This is agent judgment, the same posture as reading the instrument for gates in step 9a — read the actual column names/values, don't guess from a fixed name list. Blank/not-found on any of the three skips the whole feature, no tab, no error.
10. **Window "Entity & Country" — up to 2 tabs, one atomic fact each, standard template:**
    - **Entity**: "The entity here is `<Label>`, in place of `<column>`. I'll use this name across the report and tracking sheet."
    - **Country**: "Data collection country is `<Country>`. Timezone is `<abbrev>` (`<city, country>`, UTC`<offset>`)." Skipped when step 6 already resolved it.
11. **Window "Completion Signals" — up to 3 tabs, fires right after Entity & Country** (needs the parsed form from step 7b and profiled roles from step 7c, both already available by then):
    - **Gating & Reasons** (only when step 9a found at least one credible gate): states the guessed gate question(s) in form order with their labels (e.g. `hh_present` "Household present?", `consent` "Consents to interview?"), and that a failing answer's value becomes the non-completion "Reason" for that respondent — the first failed gate wins, since later gates were never reached. If step 9 instead found roster/primary_secondary (no gate), state that signal here the same way the old Completion tab did (name the file/column, the downstream filtering effect) — when roster wins and step 9b needed to resolve a strata-column name mismatch, also state that mapping (e.g. "the roster's `village_name` column matches the survey data's `village`"). If more than one signal type is genuinely available at once, do NOT silently pick — name the conflict explicitly and flag it as needing a closer look. Options: `"Looks good!"` / `"The list needs edits (I will type in Other)"`.
...
6. **Window "Dates, Variables & GPS" — up to 4 tabs:**
   ...
   - **GPS threshold** (M8): the guessed distance threshold (default 300m). **Conditional**: only when GPS data exists AND M8 isn't already resolved off (via A1's Instructions or a pre-filled `modules.yaml` — Operating Principle 19). Skip silently when M8 is off; nothing to ask.
...
7. **Window "Media, Map & Grouping" — up to 4 tabs:**
   ...
   - **Map focus**: the default (Country). **Conditional, same gate as GPS threshold above**: the map only exists when M8 is on (`write_html_report()` ties the map's rendering directly to `modules$M8$on`, not just GPS-data presence — declining the GPS check now hides the map too, so a map-focus question when there's no map to focus is exactly the "asking about something already ruled out" bug this rule exists to prevent). Skip together with GPS threshold.
   ...
   If both GPS threshold and Map focus are skipped, this window still has Media columns + Group label (2 tabs) — never fires with fewer than that, so it's never worth backfilling elsewhere.
...
9. **If the user answers Other on Extra checks:** propose a check name + `hfc/code/checks/<name>.R`, confirm briefly in chat, implement and register under M10/`custom`, and write its ≤3-sentence plain-English description to `hfc/config/module_notes.yaml` (`custom.<name>.label` / `.description`) so `hfc/outputs/<MMDD>_HFCs.html` can show it under the M10 section (schema in `references/check_modules.md`).
10. **Generate clean variable labels (`var_labels`) — mandatory, every run, agent judgment only, no `AskUserQuestion` confirmation** (same posture as step 4's M7 extra-vars pick): by this point every column that can actually surface in a finding or the tracking sheet is known — the unified important-variables shortlist + M7's extra missingness vars (step 4), the confirmed media columns (M11, step 7), the confirmed assent/consent/audio columns (M12, step 8), and the duplicate-key/form-version columns (M2/M3, step 5). For each, read the raw column name and its instrument question label (already available from `form_text.R`'s dump, A1 step 7b) and write a short, clean, human-readable label — neither the bare column name nor the verbatim question text (e.g. `price_rice_kg` + "What is the current market price of rice in kg" → "Rice price (kg)"). Write directly to `hfc/config/role_map.yaml`'s `var_labels: {colname: "Clean Label", ...}`. Falls back to the raw column name wherever a column has no entry (e.g. a rebuild against an older config) — never blocks a build. An RA who disagrees with a label edits the file directly, same as `missingness_extra_vars`.
11. Apply any chat corrections from the three module-confirmation windows directly to the already-written `hfc/config/modules.yaml` (edit the specific keys in place — its comments and every other module's settings stay untouched), then write the remaining `hfc/config/role_map.yaml` fields: `important_vars`, `missingness_extra_vars`, `var_labels`, `last_date`, `treatment_control_col`, `geo_group_col`, `geo_group_opted_in`, `map_focus`, `group_label`. If the user said they edited `modules.yaml` directly instead (Wrap-up window's Structure tab), re-read it now rather than trusting your own draft.
...
6. **Draft the Summary narrative** — see `assets/summary_message_example.md` for the calibration target (a real, annotated example, calibrated against an actual field RA's daily Slack update). **The summary reports on the most recent day of data collection ONLY — never the overall/cumulative picture — with exactly one exception: completion**, which always states progress against the full target sample alongside that day's own count. HFCs are typically drafted right after data collection or early the next day, so the point is to flesh out what THAT DAY's data actually showed, not to re-summarize everything to date. Read `hfc/outputs/issues.csv` and draft a Slack-register message, **lead sentences first, a numbered list after**. Length follows however many real, distinct points the latest day actually produces — don't pad, but don't compress real points just to keep it short:
   1. Names real places/entities/numbers with specificity — never "several schools," always "17 schools" / "School ID 4 Gashanga."
   2. **Lead — completion & reasons, outliers, duration, duplicates, enumerator roster, in that order, as plain sentences (not a list) — every one of these scoped to the last day's own findings (via the Date column, now reliable) except 2a:**
      ...
      c. **Outliers worth a second look** (M6), from that day's submissions only — narrate a genuine flag in plain language with its actual value, and when you don't have enough local-context judgment to be sure it's wrong, pose it as a plausibility question rather than a flat assertion, same as a field RA would.
      d. **That day's own survey duration** — mean and median in minutes, from the already-computed "Survey duration on this day" block (Last Day tab), not the overall/cumulative duration stats.
      ...
      f. **Enumerator roster for the latest day** (M1's `stats$by_enum_latest_day`) — every active enumerator's attempted-vs-completed tally, plus an explicit, actionable call-out (named directly, with their last-active date and a concrete ask — e.g. an upload-by deadline) for anyone with zero submissions that day. Not just a below-target flag: the full roster, so the reader sees who did what, not only who's in trouble.
   3. **"Other issues:" numbered list, after the lead** — that day's own remaining row-level findings (M5, M9's within-submission check, M3, M8, etc.), plus any dateless/structural finding (M7, M9's by-enumerator check, M11/M12 column-empty) that's genuinely NEW since the previous report — check `issue_tracking.xlsx`'s Status and the most recently dated `<MMDD>_HFCs.html` already in `hfc/outputs/` (if any) to tell "new" from "already known." Translate a dataset-level finding (e.g. M11's "this column is empty") into entity/group-level specifics where possible ("were not available in N schools — schools X, Y, Z"), not merely restating the raw one-line finding. Write this as a real numbered list (`1.`, `2.`, ...) — `render_summary_narrative()` (`build_outputs.R`) turns numbered lines into a real `<ol>`, not literal digits in a wall of text. One item per theme, same "name real entities/counts" rule as the lead. Never fabricate detail the pipeline's own findings/data don't support (e.g. photo content) — no check module inspects media content, only whether a media column is entirely empty (M11).
