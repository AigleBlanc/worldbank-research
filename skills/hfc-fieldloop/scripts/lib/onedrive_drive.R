...
# team, edited collaboratively by the agent, RA, and field team. OneDrive is
# required and is the sole store (see scripts/lib/issue_store.R, which builds
# the actual fetch/commit logic on top of the primitives here, plus
# require_onedrive_ready() which every user-facing script calls first). The
# local-fallback path (hfc/output/issue_tracking.xlsx) still exists in code
# but is internal/test-only — never reachable from any product code path.
#
...
# folder/file names — there is no per-project override. 
