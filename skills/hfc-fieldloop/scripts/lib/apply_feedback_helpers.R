...
apply_one_fix <- function(project_root, finding_id, corrections_text, skill_dir = NULL, date = Sys.Date()) {
  ...
  .update_clone_row(project_root, skill_dir, date, finding_id, "Resolved", corrections_text)
...
mark_needs_review <- function(project_root, finding_id, skill_dir = NULL, date = Sys.Date()) {
  .update_clone_row(project_root, skill_dir, date, finding_id, "Needs Review")
  list(status = "ok")
}
