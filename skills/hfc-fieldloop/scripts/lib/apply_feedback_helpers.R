...
clone_for_resolution_pass <- function(project_root, skill_dir = NULL, date = Sys.Date(), force_local = FALSE) {
  ctx <- fetch_issue_tracking(project_root, skill_dir = skill_dir, force_local = force_local)
...
list_open_commented_rows <- function(project_root, skill_dir = NULL, date = Sys.Date(), force_local = FALSE) {
...
apply_one_fix <- function(project_root, finding_id, corrections_text, skill_dir = NULL, date = Sys.Date(),
  ...
  fixed_ds <- env$fix(ds)
  out_path <- write_intermediate(fixed_ds, project_root, stem, ext)

  .update_clone_row(project_root, skill_dir, date, finding_id, "Resolved", corrections_text, force_local = force_local)
...
mark_needs_review <- function(project_root, finding_id, skill_dir = NULL, date = Sys.Date(), force_local = FALSE) {
  .update_clone_row(project_root, skill_dir, date, finding_id, "Needs Review", force_local = force_local)
  list(status = "ok")
}
