...
# device code if no browser is available) so you can sign in with your own
# Microsoft 365 account — this connects to YOUR OWN OneDrive, not a shared
# site; no SharePoint site or Team needs to exist. Microsoft365R/AzureAuth
# then caches the resulting token locally; every later run of
# run_setup_build.R (including ones Claude Code triggers non-interactively)
# reuses that cached token and refreshes it silently, with no further
# prompts. Share the resulting folder with collaborators afterward via
# OneDrive's own "Specific people" sharing UI.
#
...
# Optional project_root arg — only matters if a project-level onedrive.json
# overrides the skill's shared default.
...
if (!isTRUE(cfg$found)) {
  stop(
    ...
    "hfc/config/onedrive.json for a single project) and set ",
    "\"enabled\": true (plus folder_path/main_file/audit_file if you want ",
    "non-default names) before running this."
  )
}
