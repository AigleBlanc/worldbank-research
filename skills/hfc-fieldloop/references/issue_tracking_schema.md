...
| **Status** | — | see below |
| Issue Category | `category` | which check category flagged this — written as a **human-readable label** (e.g. "Low completion", not `low_completion`), via `category_label()`/`CATEGORY_LABELS` in `scripts/lib/build_outputs.R`; an unmapped category (e.g. an M10 custom check) falls back to a Title-Case-from-snake_case transform of the raw value |
