...
| F4 | Applying a fix to a row that is not `Status=Open` with a non-empty Field Team Comment (the sole eligibility gate); legacy `status`/`resolved` pairs accepted only when migrated |
...
| F18 | Enabling M11 without confirming the media-indicating columns (audio/image filename columns, plus any qualitative open-text columns) in the GPS+Media tab when they exist in the data |
...
| F25 | Skipping the last-date-of-data-collection guess in the Timing tab (A2, Window B) — drives the Last Day tab |
