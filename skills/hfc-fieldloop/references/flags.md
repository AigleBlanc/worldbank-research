...
| F17 | Running per-row media file-hygiene checks (missing/tiny/duration/duplicate/extension) — M12 only ever flags a media-indicating column that's completely empty across every surveyed row; never re-add on-disk file checks |
| F18 | Enabling M12 without confirming the media-indicating columns (audio/image filename columns, plus any qualitative open-text columns) in the GPS+Media tab when they exist in the data |
...
| F21 | Writing product artifacts outside `<Code Output Directory>/hfc/` (config, instruments, registry, outputs, code — with code/checks, code/resolutions — project.yaml) |
| F22 | Flagging child/nested question blanks as missing when parent skip-logic makes them expected |
| F23 | Skipping or silently auto-answering the "Extra checks" tab (A2, Window C) after module confirmation — it must fire every run |
...
| F25 | Skipping the last-date-of-data-collection guess in the Timing tab (A2, Window B) — drives report-wide bold-highlighting and the Last Day tab |
...
| F27 | Getting the duplicate-check key wrong without it being visibly correctable — it's auto-resolved in A1 (Entity ID alone if already unique, else + detected round/wave-like candidates) and restated in Window B tab (a) precisely so a wrong resolution can be corrected there |
...
| F31 | Skipping the mandatory important-variables shortlist step (A2) before build, or letting M6/M9/M10 fall back to their old independent auto-pools when a unified list was never confirmed |
...
| F34 | Letting incomplete/non-surveyed rows (per a confirmed "status" completion signal) leak into any M2-M13 or M10 finding — they must be filtered out before those modules run; M1 alone sees the unfiltered data |
| F35 | Confusing assent and consent, or bundling the Consent tab (A2, Window C) with any other module's confirmation |
