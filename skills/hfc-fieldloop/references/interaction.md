...
Same gates and semantics as the Cursor FieldLoop product; only the tool name and option limits differ.
...
| Pipeline B proceed | After summarizing feedback | Proceed with accepted rows (recommended) |
| Fix outline | Before apply | Confirm fix plan (recommended) |
