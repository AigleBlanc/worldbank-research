...
| **Setup** (A1) | Data found, Entity, Country, Specific instructions | Immediately after data discovery, before any module work | 4 tabs, one fact each — data found (paths + row/column/date-range, plus a project description/PAP doc if found — read silently for background context in step 1b, not a separate tab), entity label, country + timezone, an open "anything I should know?" catch-all (asked here, before role profiling, so an answer can actually shape it) |
| **Completion** (A1, conditional) | Completion | Immediately after Setup, only when a completion signal is detected | 1 tab — states the guess (or an explicit multi-signal conflict) plus the downstream filtering effect. Split out of Setup purely because Setup is already at its 4-tab cap without it |
