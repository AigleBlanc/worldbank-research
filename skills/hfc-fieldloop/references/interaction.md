...
Windows are ordered above by when they fire, not alphabetically. On a first-time clean run, only Setup → Entity & Country → Keys & Hours → Dates/Variables/GPS → Media/Map/Grouping → Wrap-up → Proceed → Commit actually appear (7 windows) — Config reuse and Merge confirm are rebuild-only conditionals, and GPS threshold/Map focus can drop out entirely per rule 12.
