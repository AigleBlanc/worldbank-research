...
| **Setup** (A1) | Data found, Entity, Country, Completion (conditional) | Immediately after data discovery, before any module work | Up to 4 tabs, one fact each — data found (paths + row/column/date-range), entity label, country + timezone, completion signal (only if detected; states the downstream filtering effect) |
