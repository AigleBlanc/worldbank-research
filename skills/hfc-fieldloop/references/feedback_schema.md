...
Collaboration surface: **two Excel files in a shared OneDrive/SharePoint folder** (see `assets/lib/onedrive.json`) plus local twin `hfc/output/feedback_sheet.xlsx`.
...
1. Defaults under `hfc-fieldloop/assets/lib/`:
   - `onedrive.json` — placeholders (`YOUR_TENANT`/`YOUR_SITE`) only
   - `onedrive.example.json` — copy template into project config, e.g.:
     ...
     ```
2. **Real site/folder:** project `hfc/config/onedrive.json` (required for OneDrive sync).
   Loader preference: `hfc/config/onedrive.json` → legacy `config/onedrive.json` → `assets/lib/onedrive.json` (first with a real `site_url` wins; placeholders → local twin / `--no-onedrive`).
3. **Auth:** delegated, one-time interactive sign-in — no secrets stored in this package (unlike the old Google service-account key). Run `Rscript scripts/onedrive_auth_setup.R` **once, yourself, in a normal interactive R/RStudio session** — this cannot be done from inside a non-interactive Claude-Code-driven run. `Microsoft365R`/`AzureAuth` then cache the token locally and refresh it silently on every later run.
4. Share the OneDrive **folder** (not the individual files) with collaborators via the normal OneDrive/SharePoint "Specific people" sharing UI, once, before relying on this — the code never mints its own share links, it just uploads into whatever folder access has already been configured.
