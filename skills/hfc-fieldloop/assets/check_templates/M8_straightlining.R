...
# Standalone, runnable script: reproduces this project's M8 (Straightlining)
# findings using the same shared logic the build itself calls. Copied into
# hfc/code/checks/M8_straightlining.R at build time with the skill/code_output_dir lines below
...
#
# Usage: Rscript hfc/code/checks/M8_straightlining.R
...
ds <- ds_for_module_selection(ds, "M8", modules)

res <- check_m8(ds, roles, modules)
res$findings <- dedupe_finding_ids(res$findings)
message("M8 (Straightlining): ", nrow(res$findings), " findings")
