...
check_m8_gps <- function(ds, roles, threshold_m = 300) {
  xc <- roles$x; yc <- roles$y; sch <- roles$school
  if (any(is.null(c(xc, yc, sch))) || any(is.na(c(xc, yc, sch)))) return(empty_findings())
  if (!all(c(xc, yc, sch) %in% names(ds))) return(empty_findings())
