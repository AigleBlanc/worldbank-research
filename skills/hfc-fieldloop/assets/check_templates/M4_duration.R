# HFC FieldLoop generated check: M4
# Standalone, runnable script: reproduces this project's M4 (Survey Duration)
# findings using the same shared logic the build itself calls. Copied into
# hfc/code/checks/M4_duration.R at build time with the skill/code_output_dir lines below
...
#
# Usage: Rscript hfc/code/checks/M4_duration.R
...
ds <- ds_for_module_selection(ds, "M4", modules)

res <- check_m4(ds, roles, modules)
res$findings <- dedupe_finding_ids(res$findings)
message("M4 (Survey Duration): ", nrow(res$findings), " findings")
print(as.data.frame(res$findings))
message("--- stats: overall / by_section / by_enumerator (minutes) ---")
print(res$stats)
