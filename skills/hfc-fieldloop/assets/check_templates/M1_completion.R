# HFC FieldLoop generated check: M1
# Standalone, runnable script: reproduces this project's M1 (Completion)
# findings using the same shared logic the build itself calls. Copied into
# hfc/code/checks/M1_completion.R at build time with the skill/code_output_dir lines below
...
#
# Usage: Rscript hfc/code/checks/M1_completion.R
...
ds <- ds_for_module_selection(ds, "M1", modules)

res <- check_m1(ds, roles, modules)
res$findings <- dedupe_finding_ids(res$findings)
message("M1 (Completion): ", nrow(res$findings), " findings")
print(as.data.frame(res$findings))
message("--- stats: overall / by_group / by_enumerator / by_date ---")
print(res$stats)
