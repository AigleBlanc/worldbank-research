# HFC FieldLoop generated check: M3
# Standalone, runnable script: reproduces this project's M3 (Form Version)
# findings using the same shared logic the build itself calls. Copied into
# hfc/code/checks/M3_form_version.R at build time with the skill/code_output_dir lines below
...
#
# Usage: Rscript hfc/code/checks/M3_form_version.R
...
ds <- ds_for_module_selection(ds, "M3", modules)

res <- check_m3(ds, roles, modules)
res$findings <- dedupe_finding_ids(res$findings)
message("M3 (Form Version): ", nrow(res$findings), " findings")
print(as.data.frame(res$findings))
if (!is.null(res$stats)) { message("--- stats: by version ---"); print(res$stats) }
