...
[Why this works: a full daily roster, not just a below-target flag — every enumerator active on the latest day gets an attempted-vs-completed tally (computed fresh from the completed dataset/`hfc/outputs/issues.csv` for that day — there's no dedicated pre-built stats table for this in v2), so the reader sees the whole picture, not just who's in trouble. The real value is the **explicit, actionable call-out for anyone who submitted nothing** — named directly, with their actual last-active date and a concrete ask (a real deadline, in both local and reference time zones) rather than a vague "please follow up." This still fires even when `daily_target_per_enum` was never confirmed — it's a roster of who did what, not a threshold check.]

> Other issues:
> 1. Attendance and test-score sheets were not available for 3 schools today - schools 141, 157, and 176.
> 2. 2 interviews at School 12 today were flagged more than 300m from the site's other submissions (advanced GPS flagging is on for this project).
> 3. The classroom-observation photo column has been completely empty since it first appeared in the data (newly noticed this build) - likely a form/coding issue worth checking.

[Why this works: a REAL numbered list (`1.`, `2.`, `3.` — `render_summary_narrative()` turns this into an actual `<ol>`, not literal digits sitting in a wall of text), one theme per item, same "name real entities/counts, never vague" rule as the lead sentences — this is where everything that ISN'T completion/reasons/outliers/duration/duplicates/enumerators belongs: that day's own remaining row-level findings (M5, M8's within-submission check, M3, plus M10 GPS Map's advanced flag if this project turned it on — most don't, so M10 usually contributes nothing here), plus any dateless/structural finding (M8's by-enumerator check, M13/M14 column-empty) that's genuinely NEW since the previous report — item 3 is exactly that case: a structural finding with no "day" of its own, but worth surfacing because it's newly noticed, not because it happened "today." The attendance/test-score gap (item 1) is translated from M13's (dataset-level) "this column is completely empty" finding — and, on a project with M7's advanced enumerator-flagging on, M7's missingness findings too — into a school-by-school list a field team can actually act on — never just restating the raw one-line finding verbatim, that tells nobody which schools to follow up with.]
